...
const API_KEY = process.env.XAI_API_KEY || process.env.GROQ_API_KEY || process.env.OPENAI_API_KEY || ''
const BASE_URL = (
...
const MODEL =
  ...
  (IS_GROQ ? 'llama-3.3-70b-versatile' : IS_XAI ? 'grok-4.5' : 'llama-3.3-70b-versatile')
...
const LANGUAGE_HINT = {
  ...
  roman: 'Write only in natural Roman Urdu. Never use Urdu script. Keep wording natural and readable in Roman Urdu.'
}
...
const LANGUAGE_MIX_RULE = {
  ...
  roman: 'SCRIPT RULE: in Roman Urdu mode use only Latin letters for every word. Never use Arabic-script (Urdu) characters anywhere in the response.'
}
...
const QUESTION_EXAMPLE = {
  ...
  roman: 'Mere baar baar honay walay sar dard ki mumkin wajah kya ho sakti hai?'
}
...
const SAFETY_BY_LANG = {
  ...
  roman: 'SEHAT LINK aap ki maloomat ko kisi qualified healthcare professional se baat karne ke liye munazzam karne mein madad karta hai. Yeh medical diagnosis nahi deta aur na hi daktar ki jagah leta hai. Agar aap ko medical emergency hai to foran apni local emergency services se raabta karein.'
}
...
async function extractPdfText(buffer) {
  try {
    ...
    console.warn('[sehat-link] pdf text extraction skipped:', err.message.slice(0, 160))
    return null
  }
}
...
function visionCapable() {
  ...
  return m.includes('vision') || m.includes('4o') || m.includes('o3') || m.includes('o4') || m.includes('gemini') || m.includes('grok')
}
...
async function chat(messages, { json = true } = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`HTTP ${res.status} from ${PROVIDER_LABEL} (${BASE_URL}/chat/completions): ${text.slice(0, 400)}`)
  }
...
function asList(v) {
  if (Array.isArray(v)) {
    const items = v.map((x) => asString(x)).filter((x) => x && x !== 'Not provided')
...
/* ------------------------------------------------------------------ */
const SPECIALISTS = [
  { keys: ['headache', 'sar dard', 'سر', 'migraine', 'dizziness', 'چکر'], name: 'Neurology', why: 'Your reported symptoms may be appropriate to discuss with a neurological specialist.' },
  { keys: ['bp', 'blood pressure', 'pressure', 'دباو', 'heart', 'دل', 'chest pain', 'palpitation'], name: 'Cardiology', why: 'Your reported symptoms may be appropriate to discuss with a cardiology specialist.' },
  { keys: ['sugar', 'diabetes', 'diabetic', 'شوگر', 'ذیابیطس'], name: 'Endocrinology', why: 'Your reported symptoms may be appropriate to discuss with an endocrinology specialist.' },
  { keys: ['cough', 'khaansi', 'کھانسی', 'breath', 'saans', 'دم', 'chest', 'phlegm', 'baalgham'], name: 'Pulmonology', why: 'Your reported symptoms may be appropriate to discuss with a pulmonology specialist.' },
  { keys: ['joint', 'jor', 'جوڑ', 'knee', 'ghutna', 'گھٹنا', 'back pain', 'kamr'], name: 'Orthopedics', why: 'Your reported symptoms may be appropriate to discuss with an orthopedics specialist.' },
  { keys: ['skin', 'جلد', 'rash', 'khujli', 'خارش', 'itching'], name: 'Dermatology', why: 'Your reported symptoms may be appropriate to discuss with a dermatology specialist.' },
  { keys: ['stomach', 'pet', 'پیٹ', 'acid', 'گیس', 'gas', 'diarrhea', 'dast', 'اسہال', 'vomit', 'qay'], name: 'Gastroenterology', why: 'Your reported symptoms may be appropriate to discuss with a gastroenterology specialist.' },
  { keys: ['eye', 'ankh', 'آنکھ', 'vision', 'nazar'], name: 'Ophthalmology', why: 'Your reported symptoms may be appropriate to discuss with an ophthalmology specialist.' },
  { keys: ['ear', 'kan', 'کان', 'hearing', 'sunai'], name: 'ENT (Ear, Nose & Throat)', why: 'Your reported symptoms may be appropriate to discuss with an ENT specialist.' },
  { keys: ['fever', 'bukhar', 'بخار', 'malaria', 'ملیریا', 'typhoid', 'ٹائیفائیڈ'], name: 'General Medicine', why: 'Your reported symptoms may be appropriate to discuss with a general medicine physician.' }
]
...
function demoBrief(payload, documents) {
  ...
  const sentences = description
    ...
    .map((s) => s.trim())
    .filter((s) => s.length > 2)
  const symptoms = sentences.length ? sentences.slice(0, 6) : ['Reported concern as described by the patient.']
  ...
  const docFindings = documents.length
    ...
    : ['Not provided']
  const medications = /medication|medicines|dawa|دوا|tablet|insulin|metformin|atorvastatin|amlodipine/i.test(description)
  ...
  const brief = {
    ...
    doctorBrief: [
      ...
      'Available documentation: ' + (documents.length ? `${documents.length} uploaded report${documents.length > 1 ? 's' : ''}` : 'Not provided'),
      'Current medications: ' + (medications[0] === 'Not provided' ? 'Not provided' : 'Referenced — confirm list with patient.'),
...
export async function generateBrief(payload) {
  ...
  if (!API_KEY) {
    if (DEMO_ALLOWED) {
      console.log('[sehat-link] SEHAT_DEMO_MODE=true — returning offline demo brief (no API key).')
  ...
  try {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      if (!issue) break
      console.warn(`[sehat-link] script purity issue (${payload.language}), corrective retry ${i + 1}/2.`)
      const corrected = await chat([...messages, { role: 'user', content: issue }], { json: true })
      brief = normalize(extractJson(corrected), payload.language)
    }

    return brief
  } catch (err) {
    console.error(`[sehat-link] Grok API call failed (${PROVIDER_LABEL}, model=${MODEL}):`, err.message)
    if (err instanceof ProviderError) throw err
    throw new ProviderError(`Grok API request failed: ${err.message}`, { status: 502, code: 'AI_PROVIDER_ERROR' })
  }
}